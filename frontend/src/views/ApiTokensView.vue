  ...
  <section class="token-page">
    ...
    <el-table v-loading="loading" :data="records" size="small" stripe>
      ...
      <el-table-column label="操作" width="190" fixed="right"><template #default="{ row }">
        ...
        <el-button link type="warning" size="small" :disabled="row.revoked_at !== null || busy" @click="toggle(row)">{{ row.enabled ? '禁用' : '恢复' }}</el-button>
...
const form = reactive<TokenInput>({ name: '', note: '', expires_at: null, rate_per_minute: 10 })
...
function openCreate() { editId.value = ''; Object.assign(form, { name: '', note: '', expires_at: null, rate_per_minute: 10 }); expiry.value = null; editing.value = true }
function openEdit(row: ApiToken) { editId.value = row.id; Object.assign(form, { name: row.name, note: row.note, rate_per_minute: row.rate_per_minute, expires_at: row.expires_at }); expiry.value = row.expires_at === null ? null : new Date(row.expires_at * 1000); editing.value = true }
async function save() {
